...
def _create_or_extend_private_grad(param: torch.Tensor, summed_clipped_grad: torch.Tensor, accumulate_private_grad = True) -> None:
    ...
    if hasattr(param, "private_grad"):
      if accumulate_private_grad == True:
        ...
        noise = torch.normal(mean=0, std=param.noise,size=param.size(), device=param.device, dtype=param.dtype)
        param.private_grad = summed_clipped_grad.detach()+ noise
        end = Decimal(time.time())
        config.shape_list.append(param.size())
        config.std_list.append(param.noise)
        config.global_gaussian = config.global_gaussian + end - start
        config.count_total_gaussian = noise.numel() + config.count_total_gaussian
