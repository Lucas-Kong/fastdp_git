...
for shape in shape_list:
    # Convert torch.Size to tuple and ensure the values are integers
    shape_tuple = tuple(int(dim) for dim in shape)  # Ensure each dimension is an integer
    samples = sample_discrete_gaussian(scale=1, shape=shape_tuple, dtype=tf.int32)
T1 = time.time()
print(T1 - T0)